...
public class Roteiro {
    ...
    private static boolean attack(Personagem jogador,Inimigo enemy) throws InterruptedException {
        ...
        int defesaEnemy;
        int vidaEnemy;
        int danoTotal = (jogador.getDano() + dados(20, 0));

        if(enemy.getVida() > 0){
            if(enemy.getDefesa() > danoTotal){
                defesaEnemy = enemy.getDefesa() - danoTotal;
                enemy.setDefesa(defesaEnemy);
                vidaEnemy = enemy.getVida();
                enemy.setVida(vidaEnemy);

                System.out.println("O inimigo tomou um dano de 0 pontos de vida, sua defesa te protegeu.\n");
            }
            else {
                danoTotal -= enemy.getDefesa();
                enemy.setDefesa(0);
                vidaEnemy = enemy.getVida() - danoTotal;
                enemy.setVida(vidaEnemy);

                System.out.println("O inimigo tomou um dano de " + danoTotal + " pontos de vida.\n");
            }
        } else {
            System.out.println("O inimigo tomou um dano de " + (jogador.getDano() + dados(20, 0)) + " pontos de vida.\n Agora posui 0 de vida" );
        }
    ...
    private static void enemyattack(Personagem jogador, Inimigo enemy, Escolhas escolha) throws InterruptedException {
        ...
        int defesaJogador;
        int vidaJogador;
        int danoTotal = (enemy.getDano() + dados(20, 0));

        //muitas repetições, se der tempo ajustar nas classes ou nas funções

            if(jogador.getDefesa() > danoTotal){
                defesaJogador = jogador.getDefesa() - danoTotal;
                jogador.setDefesa(defesaJogador);
                vidaJogador = jogador.getVida();
                jogador.setVida(vidaJogador);

                System.out.println("Você tomou um dano de 0 pontos de vida, sua defesa te protegeu.\n");
            }
            else {
                danoTotal -= jogador.getDefesa();
                vidaJogador = jogador.getVida() - danoTotal;
                jogador.setVida(vidaJogador);
                jogador.setDefesa(0);

                System.out.println("Você tomou um dano de " + danoTotal + " pontos de vida.\n");
            }
    ...
    public static void roteiro(Personagem jogador, Inimigo enemy, Escolhas escolha) throws InterruptedException {
